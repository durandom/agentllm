...
class RHDHTesterConfigurator(AgentConfigurator):
    ...
    def _build_agent_instructions(self) -> list[str]:
        ...
        return [
            ...
            "- **dynamic-plugins.yaml**: Lists enabled plugins. Structure:",
            "  - `plugins`: List of plugin objects.",
            "  - `package`: `oci://<image>:<tag>!<plugin-name>`, `npm-package@version`, or `https://...tgz`.",
            "  - `disabled`: Set to `false` to enable.",
            "  - `pluginConfig`: specific configuration for the plugin.",
            "- **Frontend Plugin Wiring** (Critical for UI plugins):",
            "  - Configured in `pluginConfig` under `dynamicPlugins.frontend.<package-name>`.",
            "  - `dynamicRoutes`: Define routes and bind them to menu items.",
            "  - `menuItems`: Define sidebar navigation items.",
            "  - `mountPoints`: Proper placement of the plugin in the UI.",
            "  - `appIcons`: Custom icons for menu items.",
            "- **General**: Do not delete existing unrelated configuration unless explicitly asked.",
            "",
            # "## Tool Usage",
